...
export default async function StrategyDetail({ params }: { params: { id: string } }) {
  const s = await backend<Strategy>(`/api/strategies/${params.id}`)
  const recs = await backend<RecommendationResponse[]>(`/api/recommendations?strategy_id=${params.id}`)
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4 space-y-2">
          <p>{s.description}</p>
          <StrategyActions strategyId={s.id} />
        </CardContent>
      </Card>
